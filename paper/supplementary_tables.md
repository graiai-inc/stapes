...
## Table S1. Fusion algorithm comparison on the pairing of the two best solo on-device models.
...
## Table S4. Per-model error rates for the highest-error medications.

Alignment-based error rates (the same normalization and jiwer substitute/delete scoring as the main clinical term recall analysis) for the six highest-error drug-dataset pairs named in the main text, computed over exactly the paper's 12 on-device and 5 cloud configurations (including AWS on OSCE) by `scripts/recompute_paper_drug_rates.py`. Cells show errors/reference occurrences (error rate); the pooled rows sum errors and occurrences across the models in each mode (a micro-average, not a mean of per-model rates). Per-model denominators are small for the PriMock57 and OSCE drugs (5 to 29 occurrences), so individual cells carry wide uncertainty while the pooled rows are stable. The per-model rows show the mode split is not an artifact of weak models in the on-device pool: the three best-WER on-device models (Parakeet-TDT-0.6b-v2, Whisper distil-v3.5, Whisper turbo) erred on 50% to 100% of these drug occurrences, above every cloud service on every drug, while the lowest on-device rates on some drugs belong to MedASR, whose overall WER makes it unusable as a sole transcription model. Category-level context: drug names were the highest-error UMLS category in 43 of 50 model-dataset evaluations; the exceptions are AWS on the psychiatric dataset (anatomy, marginally), MedASR on PriMock57 (findings), and Zipformer plus four cloud services on OSCE (procedures, symptoms, or conditions), where absolute cloud drug error rates are low.

Data: `supplementary_table_S4_drug_rates.csv`.

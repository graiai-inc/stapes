...
I submit "Privacy-Preserving On-Device Speech Recognition Achieves Near-Parity with Cloud Services for Clinical Documentation, but Both Fail on Medication Names" for consideration in JAMIA Open as a Research and Applications article, transferred from JAMIA at the editor's invitation.

The central finding is that privacy-preserving on-device speech recognition now achieves near-parity with commercial cloud services for clinical conversation transcription: the privacy, cost, and connectivity advantages of keeping patient audio on the device no longer require accepting a meaningful clinical accuracy penalty. This matters because every ambient AI clinical scribe now deploying in U.S. health systems is built on cloud ASR that transmits, and often retains, protected health information; our results show that dependence is a design choice, not a technical necessity. The same evaluation surfaces a patient-safety qualification that holds across both deployment modes: medication names are the highest-error clinical term category regardless of vendor, so any clinical ASR deployment requires explicit medication verification before notes are finalized. JAMIA has published the two most-cited systematic reviews of clinical speech recognition (Blackley et al. 2019; Hodgson and Coiera 2016), which together documented weak and inconsistent prior evidence and significant gaps in standardized evaluation; this work contributes open, reproducible empirical evidence in that gap, built using only publicly redistributable audio and code so that other groups can extend and critique it.
